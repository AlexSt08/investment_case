...
// Quill loaded client-side only
const ReactQuill = dynamic(
  async () => {
    const { default: RQ } = await import('react-quill')
    return RQ
  },
  { ssr: false, loading: () => <div style={{ minHeight: 480, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '0 0 6px 6px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>Chargement éditeur…</span></div> }
)
...
function tiptapToHtml(doc: any): string {
  ...
  return doc.content.map((n: any) => nodeToHtml(n)).join('')
}
...
function ChartModal({ onInsert, onClose }: { onInsert: (html: string) => void; onClose: () => void }) {
  ...
  const [dataset1Label, setDataset1Label] = useState('Revenus ($M)')
  const [dataset1Values, setDataset1Values] = useState('1200,1450,1680,2100,2600,3200')
  const [dataset2Label, setDataset2Label] = useState('')
  const [dataset2Values, setDataset2Values] = useState('')
  const [color1, setColor1] = useState('#c8a96e')
  const [color2, setColor2] = useState('#6366f1')

  const generateChart = () => {
    const id = `chart_${Date.now()}`
    const type = chartType === 'Ligne' ? 'line' : chartType === 'Barres' ? 'bar' : 'horizontalBar'
    const ds: any[] = [{
      label: dataset1Label,
      data: dataset1Values.split(',').map(v => parseFloat(v.trim())),
      borderColor: color1,
      backgroundColor: type === 'line' ? color1 + '20' : color1 + '90',
      tension: 0.3,
      fill: type === 'line',
    }]
    if (dataset2Label && dataset2Values) {
      ds.push({
        label: dataset2Label,
        data: dataset2Values.split(',').map(v => parseFloat(v.trim())),
        borderColor: color2,
        backgroundColor: type === 'line' ? color2 + '20' : color2 + '90',
        tension: 0.3,
        fill: false,
      })
    }
    const config = JSON.stringify({
      type,
      data: { labels: labels.split(',').map(l => l.trim()), datasets: ds },
      options: {
        responsive: true,
        plugins: {
          legend: { labels: { color: '#8a8a9a', font: { family: 'Inter' } } },
          title: title ? { display: true, text: title, color: '#f0ede8', font: { family: 'Playfair Display', size: 14 } } : undefined,
        },
        scales: {
    ...
    const html = `<div class="chart-container" style="position:relative;background:#0f0f18;border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:20px;margin:28px 0;">
  ...
  <script>
    (function(){
      ...
      var config=${config};
      if(typeof Chart!=='undefined'){new Chart(ctx,config);}
      else{
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, padding: 28, width: 520, maxHeight: '85vh', overflowY: 'auto' }}>
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {CHART_TYPES.map(t => (
              <button key={t} type="button" onClick={() => setChartType(t)} style={{ padding: '6px 14px', borderRadius: 6, border: `1px solid ${chartType === t ? 'var(--accent-border)' : 'var(--border)'}`, background: chartType === t ? 'var(--accent-dim)' : 'transparent', color: chartType === t ? 'var(--accent)' : 'var(--text-secondary)', fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 48px', gap: 8, marginBottom: 12 }}>
          <div>
            <label className="form-label">Série 1 — Nom</label>
            <input className="form-input" value={dataset1Label} onChange={e => setDataset1Label(e.target.value)} />
          </div>
          <div>
            <label className="form-label">Couleur</label>
            <input type="color" value={color1} onChange={e => setColor1(e.target.value)} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid var(--border)', background: 'none', cursor: 'pointer' }} />
          </div>
        </div>
        <div className="form-group">
          <label className="form-label">Série 1 — Valeurs</label>
          <input className="form-input" value={dataset1Values} onChange={e => setDataset1Values(e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 48px', gap: 8, marginBottom: 12 }}>
          <div>
            <label className="form-label">Série 2 — Nom (optionnel)</label>
            <input className="form-input" value={dataset2Label} onChange={e => setDataset2Label(e.target.value)} placeholder="Marge EBITDA (%)" />
          </div>
          <div>
            <label className="form-label">Couleur</label>
            <input type="color" value={color2} onChange={e => setColor2(e.target.value)} style={{ width: '100%', height: 38, borderRadius: 6, border: '1px solid var(--border)', background: 'none', cursor: 'pointer' }} />
          </div>
        </div>
        <div className="form-group">
          <label className="form-label">Série 2 — Valeurs</label>
          <input className="form-input" value={dataset2Values} onChange={e => setDataset2Values(e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} placeholder="32,35,38,41,44,47" />
        </div>

        <button type="button" onClick={generateChart} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }}>
...
function CompanyRow({ entry, index, companies, onChange, onRemove }: {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 80px 32px', gap: 6, marginBottom: 8, padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 6 }}>
...
export default function CaseEditor({ caseId }: Props) {
  ...
  const loadCase = async () => {
    ...
    // Convert content: handle both TipTap JSON and HTML
    const rawContent = data.content as any
    let html = ''
    if (typeof rawContent === 'string' && rawContent.startsWith('{')) {
      try { html = tiptapToHtml(JSON.parse(rawContent)) } catch { html = rawContent }
    } else if (typeof rawContent === 'object' && rawContent !== null) {
      html = tiptapToHtml(rawContent)
    } else {
      html = rawContent ?? ''
    }
    setHtmlContent(html)
    ...
    if (data.case_companies?.length) {
      setCompanyEntries(data.case_companies.map((cc: CaseCompany) => ({
        company_id: cc.company_id, rating: cc.rating ?? '', target_price: cc.target_price ?? '', upside: cc.upside ?? '',
      })))
    }

    // Load linked tags
    const { data: caseTags } = await supabase
      .from('investment_case_tags')
      .select('tag_id')
      .eq('investment_case_id', caseId)
    setSelectedTags(caseTags?.map((t: any) => t.tag_id) ?? [])
    setLoaded(true)
  }

  const insertChart = useCallback((html: string) => {
  ...
  const createTag = async () => {
    ...
    const { data } = await supabase.from('tags').insert({ name: newTagName.trim(), slug }).select().single()
  ...
  const handleSave = async (publish = false) => {
    ...
    const payload = {
      ...
      content: htmlContent, updated_at: new Date().toISOString(),
    ...
    if (selectedTags.length > 0) {
      await supabase.from('investment_case_tags').insert(selectedTags.map(tag_id => ({ investment_case_id: savedId, tag_id })))
    }
  ...
  return (
    <>
      ...
      {/* Quill CSS */}
      <style>{`
        .ql-toolbar { background: var(--bg-elevated) !important; border: 1px solid var(--border) !important; border-radius: 6px 6px 0 0 !important; flex-wrap: wrap; }
        .ql-container { background: var(--bg-card) !important; border: 1px solid var(--border) !important; border-top: none !important; border-radius: 0 0 6px 6px !important; font-family: var(--font-body) !important; font-size: 0.95rem !important; color: var(--text-primary) !important; min-height: 480px; }
        .ql-editor { min-height: 480px; line-height: 1.8; color: var(--text-primary) !important; }
        ...
        .ql-toolbar .ql-picker-item { color: var(--text-primary) !important; }
        .ql-editor h1, .ql-editor h2, .ql-editor h3 { font-family: var(--font-display) !important; color: var(--text-primary) !important; }
        .ql-editor blockquote { border-left: 3px solid var(--accent) !important; padding: 8px 16px !important; background: var(--accent-dim) !important; color: var(--text-secondary) !important; margin: 16px 0 !important; }
        .ql-editor table { border-collapse: collapse; width: 100%; margin: 16px 0; }
        .ql-editor td, .ql-editor th { border: 1px solid var(--border); padding: 8px 12px; }
        .ql-editor pre.ql-syntax { background: var(--bg-elevated) !important; color: var(--accent) !important; border-radius: 6px !important; }
        .chart-container canvas { max-height: 320px; }
      `}</style>

      {showChartModal && <ChartModal onInsert={insertChart} onClose={() => setShowChartModal(false)} />}
        ...
        <nav style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-card)', position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <span style={{ fontSize: '0.85rem', color: 'var(--text-primary)' }}>{isEdit ? 'Éditer' : 'Nouvelle analyse'}</span>
            </div>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              {saved && <span style={{ fontSize: '0.78rem', color: 'var(--buy)', fontFamily: 'var(--font-mono)' }}>✓ Sauvegardé</span>}
              <button type="button" onClick={() => handleSave(false)} className="btn btn-ghost" disabled={saving} style={{ padding: '6px 14px', fontSize: '0.78rem' }}>
                {saving ? '…' : 'Brouillon'}
              </button>
              <button type="button" onClick={() => handleSave(true)} className="btn btn-primary" disabled={saving} style={{ padding: '6px 14px', fontSize: '0.78rem' }}>
                {saving ? '…' : '▲ Publier'}
              </button>
            </div>
          </div>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 24px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32, alignItems: 'start' }}>
          ...
          <div>
            ...
            <div className="form-group">
              ...
              <textarea className="form-textarea" value={excerpt} onChange={e => setExcerpt(e.target.value)} rows={3} placeholder="2-3 phrases résumant la thèse d'investissement…" />
            </div>
            ...
            <div className="form-group">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <button type="button" onClick={() => setShowChartModal(true)} className="btn btn-ghost" style={{ padding: '5px 12px', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <ReactQuill
                ...
                placeholder="Commencez à rédiger votre analyse ici…"
                theme="snow"
              />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
              ...
              {companyEntries.map((e, i) => (
                <CompanyRow key={i} entry={e} index={i} companies={companies} onChange={updateEntry} onRemove={removeEntry} />
              ))}
              <button type="button" onClick={addCompanyRow} className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center', fontSize: '0.78rem', padding: '7px' }}>
                + Ajouter une société
              </button>
              <Link href="/admin/societes" style={{ display: 'block', marginTop: 8, fontSize: '0.7rem', color: 'var(--accent)', textDecoration: 'none', textAlign: 'center' }}>
                Gérer l'univers de couverture →
              </Link>
            </div>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
              ...
              <div className="form-group">
                <label className="form-label">Rating global (industrie / macro)</label>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  {RATINGS.map(r => (
                    <button key={r} type="button" onClick={() => setRating(r)} className={`badge-rating ${r}`} style={{ cursor: 'pointer', opacity: rating === r ? 1 : 0.35, transition: 'opacity 0.15s', padding: '4px 10px', fontSize: '0.68rem' }}>{r}</button>
                  ))}
                </div>
              </div>
            ...
            {/* Tags thématiques */}
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', letterSpacing: '0.12em', color: 'var(--text-muted)', marginBottom: 16 }}>TAGS THÉMATIQUES</p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                ...
                {allTags.length === 0 && <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Aucun tag. Créez-en un ci-dessous.</p>}
              </div>
            ...
            {isEdit && slug && (
              <a href={`/analyse/${slug}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost" style={{ justifyContent: 'center', fontSize: '0.78rem' }}>
                ↗ Page publique
              </a>
            )}
          </div>

...
function tiptapToHtml(doc: any): string {
  if (typeof doc === 'string') {
    const t = doc.trim()
    if (t.startsWith('{') || t.startsWith('[')) { try { return tiptapToHtml(JSON.parse(t)) } catch { return doc } }
    return doc
  }
...
function ChartModal({ onInsert, onClose }: { onInsert: (html: string) => void; onClose: () => void }) {
  ...
  const generate = () => {
    ...
    const datasets: any[] = [{ label: s1Label, data: s1Values.split(',').map((v: string) => parseFloat(v.trim())), borderColor: color1, backgroundColor: chartType === 'line' ? color1 + '25' : color1 + '99', tension: 0.3, fill: chartType === 'line' }]
    if (s2Label && s2Values) datasets.push({ label: s2Label, data: s2Values.split(',').map((v: string) => parseFloat(v.trim())), borderColor: color2, backgroundColor: chartType === 'line' ? color2 + '25' : color2 + '99', tension: 0.3, fill: false })
    const cfg = JSON.stringify({ type: chartType, data: { labels: labels.split(',').map((l: string) => l.trim()), datasets }, options: { responsive: true, plugins: { legend: { labels: { color: '#8a8a9a' } }, title: title ? { display: true, text: title, color: '#f0ede8' } : undefined }, scales: { x: { ticks: { color: '#52525e' }, grid: { color: '#ffffff0a' } }, y: { ticks: { color: '#52525e' }, grid: { color: '#ffffff0a' } } } } })
      ...
      <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, padding: 28, width: 500, maxHeight: '85vh', overflowY: 'auto' }}>
        ...
        <div className="form-group"><label className="form-label">Type</label>
          <div style={{ display: 'flex', gap: 8 }}>
            {(['line', 'bar'] as const).map(t => <button key={t} type="button" onClick={() => setChartType(t)} style={{ padding: '6px 16px', borderRadius: 6, border: `1px solid ${chartType === t ? 'var(--accent-border)' : 'var(--border)'}`, background: chartType === t ? 'var(--accent-dim)' : 'transparent', color: chartType === t ? 'var(--accent)' : 'var(--text-secondary)', fontSize: '0.8rem', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>{t === 'line' ? 'Ligne' : 'Barres'}</button>)}
          </div>
        </div>
        <div className="form-group"><label className="form-label">Titre</label><input className="form-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="Revenus annuels ($M)" /></div>
        <div className="form-group"><label className="form-label">Étiquettes (virgules)</label><input className="form-input" value={labels} onChange={e => setLabels(e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 44px', gap: 8, marginBottom: 8 }}>
          ...
          <div><label className="form-label">Col.</label><input type="color" value={color1} onChange={e => setColor1(e.target.value)} style={{ width: '100%', height: 38, border: '1px solid var(--border)', borderRadius: 6, background: 'none', cursor: 'pointer' }} /></div>
        </div>
        <div className="form-group"><label className="form-label">Série 1 — Valeurs</label><input className="form-input" value={s1Values} onChange={e => setS1Values(e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 44px', gap: 8, marginBottom: 8 }}>
          <div><label className="form-label">Série 2 — Nom (optionnel)</label><input className="form-input" value={s2Label} onChange={e => setS2Label(e.target.value)} placeholder="Marge (%)" /></div>
          <div><label className="form-label">Col.</label><input type="color" value={color2} onChange={e => setColor2(e.target.value)} style={{ width: '100%', height: 38, border: '1px solid var(--border)', borderRadius: 6, background: 'none', cursor: 'pointer' }} /></div>
        </div>
        <div className="form-group"><label className="form-label">Série 2 — Valeurs</label><input className="form-input" value={s2Values} onChange={e => setS2Values(e.target.value)} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }} placeholder="32,35,38,41,44,47" /></div>
...
function CompanyRow({ entry, index, companies, onChange, onRemove }: { entry: CompanyEntry; index: number; companies: Company[]; onChange: (i: number, f: keyof CompanyEntry, v: string) => void; onRemove: (i: number) => void }) {
  return (
...
export default function CaseEditor({ caseId }: Props) {
  ...
  const checkAuth = async () => { const { data } = await supabase.auth.getUser(); if (!data.user) router.push('/admin/login') }
  ...
  const loadCase = async () => {
    ...
    if (!data) return
    setTitle(data.title); setSubtitle(data.subtitle ?? ''); setSlug(data.slug); setSlugManual(true)
    setExcerpt(data.excerpt ?? ''); setRating((data.rating as Rating) ?? ''); setHorizon(data.target_horizon ?? ''); setSectorId(data.sector_id ?? '')
    setHtml(tiptapToHtml(data.content))
    if (data.case_companies?.length) setCompanyEntries(data.case_companies.map((cc: CaseCompany) => ({ company_id: cc.company_id, rating: cc.rating ?? '', target_price: cc.target_price ?? '', upside: cc.upside ?? '' })))
  ...
  const handleSave = async (publish = false) => {
    ...
    await syncCaseCompanies(savedId!, companyEntries.filter(e => e.company_id).map(e => ({ company_id: e.company_id, rating: (e.rating as Rating) || undefined, target_price: e.target_price || undefined, upside: e.upside || undefined })))
  ...
  if (!loaded) return <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><p style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.8rem' }}>Chargement…</p></div>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 24px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32, alignItems: 'start' }}>
          <div>
            ...
            <div className="form-group"><label className="form-label">Titre *</label><input type="text" className="form-input" value={title} onChange={e => setTitle(e.target.value)} placeholder="ex: NVDA — Le supercycle de l'IA n'est pas terminé" style={{ fontSize: '1.05rem', padding: '12px 16px' }} /></div>
            <div className="form-group"><label className="form-label">Sous-titre</label><input type="text" className="form-input" value={subtitle} onChange={e => setSubtitle(e.target.value)} placeholder="Résumé en une phrase de la thèse" /></div>
            ...
            <div className="form-group"><label className="form-label">Résumé (cartes)</label><textarea className="form-textarea" value={excerpt} onChange={e => setExcerpt(e.target.value)} rows={3} placeholder="2-3 phrases résumant la thèse…" /></div>
            <div className="form-group">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
              ...
              <div className="form-group"><label className="form-label">Rating global</label>
              ...
              <div className="form-group"><label className="form-label">Horizon</label>
                <select className="form-select" value={horizon} onChange={e => setHorizon(e.target.value)}>
                  <option value="">— Sélectionner —</option>
                  {HORIZONS.map(h => <option key={h} value={h}>{h}</option>)}
                </select>
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}><label className="form-label">Secteur</label>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 20 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                {allTags.map(tag => <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)} style={{ padding: '4px 12px', borderRadius: 20, fontSize: '0.75rem', cursor: 'pointer', background: selectedTags.includes(tag.id) ? 'var(--accent-dim)' : 'transparent', border: `1px solid ${selectedTags.includes(tag.id) ? 'var(--accent-border)' : 'var(--border)'}`, color: selectedTags.includes(tag.id) ? 'var(--accent)' : 'var(--text-secondary)', fontFamily: 'var(--font-body)', transition: 'all 0.15s' }}>{tag.name}</button>)}
            ...
            {isEdit && slug && <a href={`/analyse/${slug}`} target="_blank" rel="noopener noreferrer" className="btn btn-ghost" style={{ justifyContent: 'center', fontSize: '0.78rem' }}>↗ Page publique</a>}
          </div>
        </div>
      </div>
    </>
